...
def select_all_books():
    ...
    sql='SELECT title, author, syupan, isbn FROM kaihatu_book'
...
def insert_book(title, author, syupan, isbn):
    connection = get_connection()
    cursor = connection.cursor()
    sql='INSERT INTO kaihatu_book VALUES (default, %s, %s, %s, %s)'
    
    cursor.execute(sql, (title, author, syupan, isbn))
...
def delete_book(isbn):
    connection = get_connection()
    cursor = connection.cursor()
    sql='DELETE FROM kaihatu_book WHERE isbn = %s'
...
def search_book(title):
    connection = get_connection()
    cursor = connection.cursor()
    sql = 'SELECT title, author, syupan, isbn FROM kaihatu_book WHERE title LIKE %s'
...
def update_book(title, isbn):
    ...
    sql="update kaihatu_book set title = %s where isbn = %s"
